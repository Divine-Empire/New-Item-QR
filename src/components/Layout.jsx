...
import { LayoutDashboard, Package, Settings, Menu, X, LogOut, User, Clock, History as HistoryIcon } from 'lucide-react';
...
const Layout = ({ children }) => {
    ...
    // Filter nav items based on user page access
    const allowedPages = user?.pageAccess
        ? user.pageAccess.split(',').map(p => p.trim())
        : [];
    ...
    return (
        <div className="flex bg-slate-50 h-screen supports-[height:100dvh]:h-[100dvh] w-full overflow-hidden">
            ...
            <aside
                className={clsx(
                    "fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white border-r border-light-blue-100 flex flex-col transition-transform duration-200 ease-in-out",
            ...
            >
                ...
                <div className="p-4 border-t border-light-blue-100">
                    <div className="flex items-center gap-3 px-4 py-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-light-blue-100 flex items-center justify-center text-light-blue-600">
                            <User size={18} />
                        </div>
